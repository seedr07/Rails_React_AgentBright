...
group(:red_green_refactor, halt_on_fail: true) do
  guard(
    ...
    watch(%r{^app/carriers/(.+)_service\.rb}) { |m| "test/carriers/#{m[1]}_test.rb" }
    watch(%r{^app/controllers/application_controller\.rb$}) { "test/controllers" }
    watch(%r{^app/controllers/(.+)_controller\.rb$}) { |m| "test/integration/#{m[1]}_test.rb" }
    watch(%r{^app/facades/(.+)_service\.rb}) { |m| "test/facades/#{m[1]}_test.rb" }
    watch(%r{^app/helpers/(.+)_service\.rb}) { |m| "test/helpers/#{m[1]}_test.rb" }
    watch(%r{^app/jobs/(.+)_service\.rb}) { |m| "test/jobs/#{m[1]}_test.rb" }
