class StripeSubscription
  ...
  def initialize(checkout)
    Rails.logger.debug "[STRIPE] Initializing StripeSubscription..."
    @checkout = checkout
  end
  ...
  def rescue_stripe_exception
    ...
    Rails.logger.warn "[STRIPE] Rescued Stripe exception..."
    Rails.logger.warn "[STRIPE] Exception: #{exception}"
    false
  end
  ...
  def subscription_attributes
    base_subscription_attributes.merge(coupon_attributes).merge(trial_attributes)
  end
