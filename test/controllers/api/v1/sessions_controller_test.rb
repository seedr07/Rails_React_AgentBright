...
class Api::V1::SessionsControllerTest < ActionController::TestCase
  ...
  def test_valid_email_and_password_should_be_able_to_log_in
    admin = users :admin
    post :create, params: { user: { email: admin.email, password: "welcome1" } }
    assert_response :success
  end

  def test_wrong_combination_of_email_and_password_should_not_be_able_to_log_in
    non_existent_email = "this_email_does_not_exist_in_db@example.com"
    post :create, params: { user: { email: non_existent_email, password: "welcome1" } }
    assert_response 401
